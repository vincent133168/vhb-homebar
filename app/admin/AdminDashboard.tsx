...
type Drink = { id:string; name:string; englishName:string; bar:string; city:string; category:string; price:number; image:string; ingredients:string[]; recipe:string[]; taste:string; strength:string; minutes:number; story:string; rank?:number; sourceUrl?:string };
...
export default function AdminDashboard({ userName }: { userName:string }) {
  ...
  async function deleteCocktail(drink:Drink){
    ...
    const data=await response.json();
    if(response.ok){setActiveRecipe(null);await refresh();flash(`${drink.name} 已从配方库删除`);}else flash(data.error||"删除配方失败");
    setBusy("");
  }
  ...
  return <main className="admin-shell">
    ...
    {activeRecipe&&<div className="recipe-detail-overlay" role="dialog" aria-modal="true" aria-label={`${activeRecipe.name}配方详情`} onMouseDown={(event)=>event.target===event.currentTarget&&setActiveRecipe(null)}><article className="recipe-detail-card"><button className="close" onClick={()=>setActiveRecipe(null)} aria-label="关闭配方详情">×</button><div className="recipe-detail-photo"><img src={activeRecipe.image} alt={activeRecipe.name}/><span>{activeRecipe.category==="classic"?"经典鸡尾酒":activeRecipe.category==="topbar"?"TOP 酒吧酒单":"VHB 原创"}</span></div><div className="recipe-detail-body"><header><span className="admin-kicker">RECIPE DETAIL</span><h2>{activeRecipe.name}</h2><p>{activeRecipe.englishName}</p></header><div className="recipe-facts"><span>{activeRecipe.bar}</span><span>{activeRecipe.city}</span><span>{activeRecipe.taste}</span><span>{activeRecipe.strength}</span><span>{activeRecipe.minutes} 分钟</span></div>{activeRecipe.story&&<p className="recipe-story">{activeRecipe.story}</p>}<div className="recipe-columns"><section><h3>所需材料</h3><ul>{activeRecipe.ingredients.map((item,index)=><li key={`${item}-${index}`}><i>{String(index+1).padStart(2,"0")}</i><span>{item}</span></li>)}</ul></section><section><h3>制作方式</h3><ol>{activeRecipe.recipe.map((step,index)=><li key={`${step}-${index}`}><i>{index+1}</i><span>{step}</span></li>)}</ol></section></div><footer>{activeRecipe.sourceUrl?<a href={activeRecipe.sourceUrl} target="_blank" rel="noreferrer">查看配方来源 ↗</a>:<span>VHB 配方数据库</span>}<button className="delete-recipe" disabled={busy===`delete-drink:${activeRecipe.id}`} onClick={()=>void deleteCocktail(activeRecipe)}>{busy===`delete-drink:${activeRecipe.id}`?"正在删除…":"删除该配方"}</button></footer></div></article></div>}
